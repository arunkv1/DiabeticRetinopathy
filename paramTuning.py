...
# These are the combinations of parameters that we wanted to test out for cross validation
maxIters = [10, 20, 30]
regParams = [0.05, 0.1, 0.2]
elasticParams = [0.3, 0.5, 0.7]
# opening the file to write all the results to in AWS S3
file = open("s3://final-project-657/crossVals.txt", "w")
file.write("---------- Cross validation Results ----------\n\n")
# This will go through every combination of the three parameters
count = 0
for m in maxIters:
    for r in regParams:
        for e in elasticParams:
            # writing which combination is currently being executed
            file.write("MaxIters = " + str(m) + "\n")
            file.write("regParam = " + str(r) + "\n")
            file.write("elasticNetParam = " + str(e) + "\n")
            # this will split the data into a 70% train data, and 30% test data
            trainDf, testDf = dataDf.randomSplit([0.7, 0.3], seed=10)
            print(trainDf.show())
            ...
            # This will inialize the logistic regression model with the current combination of the parameters
            lr = LogisticRegression(maxIter=m, regParam=r, elasticNetParam=e, labelCol="label")
            ...
            predAndLabels = predDf.select("prediction", "label")
            ...
            # Wrting the accuracy to the file
